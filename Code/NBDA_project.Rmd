...
plot(sort(rf$importance, decreasing=TRUE), main = "Graph of sorted importance values") 
...
# Create named vector: probe ID → gene symbol
probe_symbols <- setNames(probe_to_symbol$symbol, probe_to_symbol$probe)

# Sort importance
ordered_importance <- rf_importance[ordered_probes, , drop = FALSE]

# Get corresponding gene symbols, fallback to probe ID if not found
gene_labels <- ifelse(ordered_probes %in% names(probe_symbols),
                      probe_symbols[ordered_probes],
                      ordered_probes)

# Plot manually
barplot(ordered_importance[, 1],
        names.arg = gene_labels,
...
```{r}
indices_top200_annotated <- match(top200, annotLookup$affy_hg_u133_plus_2)
top200_annotated <- annotLookup[indices_top200_annotated,"external_gene_name"]
top200_annotated
```

```{r save top200_annotated to csv file, include=FALSE}
# save top200_annotated to txt file
write.csv(top200_annotated, file = 'output/top200_annotated.txt', quote=FALSE, row.names =FALSE, col.names=FALSE)
```
